...
final class DanmakuMaskFrame {
  const DanmakuMaskFrame({required this.viewBox, required this.blockedPath});

  final ui.Rect viewBox;

  /// viewBox 坐标系下的遮挡区（人像），由构造保证落在 viewBox 内
  final ui.Path blockedPath;
}
...
DanmakuMaskFrame? buildDanmakuMaskFrame(WebMaskFrameData data) {
  ...
  final viewBoxRect = ui.Rect.fromLTWH(
    viewBox[0],
    viewBox[1],
    viewBox[2],
    viewBox[3],
  );
  final blocked = ui.Path.combine(
    ui.PathOperation.difference,
    ui.Path()..addRect(viewBoxRect),
    allowed,
  );
  // 无人像帧：不遮挡，让驱动推 null 走 Clip.none
  if (blocked.getBounds().isEmpty) return null;
  return DanmakuMaskFrame(viewBox: viewBoxRect, blockedPath: blocked);
}
